...
from DOLLY.utils.database import get_assistant, clonebotdb
from DOLLY.utils.database.clonedb import has_user_cloned_any_bot
...
C_BOT_DESC = "Wᴀɴᴛ ᴀ ʙᴏᴛ ʟɪᴋᴇ ᴛʜɪs? Cʟᴏɴᴇ ɪᴛ ɴᴏᴡ! ✅\n\nVɪsɪᴛ: @ChatGPTMUSICBOT ᴛᴏ ɢᴇᴛ sᴛᴀʀᴛᴇᴅ!\n\n - Uᴘᴅᴀᴛᴇ: @KomalMusicUpdate\n - Sᴜᴘᴘᴏʀᴛ: @YARO_KA_ADDAA"
    ...
    if len(message.command) > 1:
        ...
        try:
            ai = Client(
                ...
                API_HASH,
                bot_token=bot_token,
                plugins=dict(root="DOLLY.cplugin"), 
            )
            await ai.start()
            bot = await ai.get_me()
        ...
        try:
            ...
            details = {
                ...
                "channel": "KomalMusicUpdate",
                "support": "YARO_KA_ADDAA",
                "premium" : False,
                "Date" : False,
            }
            ...
            await mi.edit_text(
                f"⚠️ <b>ᴇʀʀᴏʀ:</b>\n\n<code>{e}</code>\n\n**ᴋɪɴᴅʟʏ ғᴏᴡᴀʀᴅ ᴛʜɪs ᴍᴇssᴀɢᴇ ᴛᴏ @YARO_KA_ADDAA ᴛᴏ ɢᴇᴛ ᴀssɪsᴛᴀɴᴄᴇ**"
            )
    else:
